...
def generate_bookings():
    ...
    print(f"Found {len(hotels)} hotels. Generating 20 bookings for each...")
    Booking.objects.all().delete()
    Transaction.objects.all().delete()
    print("Cleared existing bookings and transactions.")

    for hotel in hotels:
        room_types = list(RoomType.objects.filter(hotel=hotel))
        if not room_types:
            print(f"Skipping Hotel '{hotel.name}' - No room types available.")
            continue

        for _ in range(20):
            # Random dates within last 6 months to next 1 month
            now = timezone.now()
            days_offset_start = random.randint(-180, 20)
            check_in_date = (now + timedelta(days=days_offset_start)).date()
            nights = random.randint(1, 5)
            check_out_date = check_in_date + timedelta(days=nights)
            
            created_at_dt = now + timedelta(days=days_offset_start - random.randint(1, 30))

            booking_status = random.choice(statuses)

            # Random room selection (1 or 2 types if available)
            ...
            # Save booking first to get an ID for BookingRoom relations
            booking.save()
            ...
            # If completed, create a Transaction to match
        ...
        print(f"Created 20 bookings for hotel ID {hotel.id}")

    print("Successfully seeded bookings data!")
